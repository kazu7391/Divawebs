...
var dvmenu = {
    ...
    'submitTopItemForm' : function (menu_id) {
        ...
        $.ajax({
            ...
            success : function (json) {
                if(json['submit'] === true) {
                    dvmenu.getItemsToContainer(menu_id);
                } else {
                    flag = false;
                    $('#form-container-bg').show();
                    $('#form-ajax-loader').show();
            ...
            success : function (json) {
                ...
                if(child_categories) {
                    for(i in child_categories) {
                        ...
                        html += '           <a class="a-config" onclick="dvmenu.showWidgetConfiguration($(this))"><i class="fa fa-pencil" aria-hidden="true"></i></a>';
                        html += '           <a class="a-delete a-config" onclick="dvmenu.deleteWidget($(this))"><i class="fa fa-times" aria-hidden="true"></i></a>';
                        html += '       </div>';
                        html += '   </div>';
    ...
    'addWidget': function() {
        ...
        if(widget_type == 'html') {
            ...
            $.ajax({
                url : 'index.php?route=diva/ultimatemenu/getLanguageData&user_token=' + user_token,
                type: 'get',
                success : function (json) {
        ...
        dvmenu.initSortMenuItem();
        dvmenu.chooseCategory();
        dvmenu.chooseProduct();
        dvmenu.initSummerNote();
        dvmenu.checkBoxClick();
    },
    ...
    'chooseCategory' : function () {
        ...
        $('.w-category').customAutoComplete({
            'source': function(request, response) {
                $.ajax({
                    url: 'index.php?route=diva/ultimatemenu/autoCompleteCategory&user_token=' + user_token + '&filter_name=' +  encodeURIComponent(request),
                    dataType: 'json',
                    success: function(json) {
