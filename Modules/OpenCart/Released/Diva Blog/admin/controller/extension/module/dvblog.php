<?php
class ControllerExtensionModuleDvblog extends Controller
{
    private $error = array();
    ...
    public function install() {
        ...
        $this->model_diva_blog->install();

        $data = array(
            'module_dvblog_post_limit'       => '10',
            'module_dvblog_meta_title'       => 'Blog',
            'module_dvblog_meta_description' => 'Blog Description',
            'module_dvblog_meta_keyword'     => 'Blog Keyword'
        );

        $this->model_setting_setting->editSetting('module_dvblog', $data, 0);
    }

    public function uninstall() {
        $this->load->model('diva/blog');
        $this->model_diva_blog->uninstall();
    }

    public function index() {
        $this->load->language('extension/module/dvblog');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if (!isset($this->request->get['module_id'])) {
                $this->model_setting_module->addModule('dvblog', $this->request->post);
            } else {
        ...
        $post_lists = $this->model_diva_blog->getAllPostList();
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('extension/module/dvblog', 'user_token=' . $this->session->data['user_token'], true),
        );
        ...
        if (isset($this->request->get['module_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            $data['action'] = $this->url->link('extension/module/dvblog', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
        }
        ...
        $this->response->setOutput($this->load->view('diva/module/dvblog', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/dvblog')) {
